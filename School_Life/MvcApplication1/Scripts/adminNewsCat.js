...
function getNewsCat(page) {
    var pageCode = page || "";
    $.ajax({
        type: "POST",
        data: "pageCode=" + pageCode,
        url: "/Admin/NewsCatPager",
        async: true,
        success: function (data) {
            ...
            var newscat = data.newscat;
            var tr = "";
            for (var item in newscat) {
                tr += "<tr>";
                tr += "<td>" + newscat[item]["newscatid"] + "</td>" + "<td>" + newscat[item]["newscatnm"] + "</td>";
                tr += "<td><input type='hidden' value=" + newscat[item]["newscatid"] + " /><a class='delnewscat'  href='javascript:void(0);'>删除</a></td>";
                tr += "</tr>";
            }
...
$(function () {
    ...
    //绑定删除商品分类事件
    $(".table").on("click", ".delnewscat", function () {
        ...
        swal({
            ...
            $.ajax({
                url: 'DelNewsCat',
                data: 'newsCatId=' + newsid,
                type: 'post',
                cache: false,
                success: function (data) {
                    data = eval("(" + data + ")");
                    var $toast = toastr['success'](data["msg"]);
                    getNewsCat(); //重新查询首页，从第一页开始
                }
            });
        });
    });
