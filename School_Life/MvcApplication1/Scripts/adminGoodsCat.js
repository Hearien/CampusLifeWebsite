...
function getGoodsCat(page) {
    var pageCode = page || "";
    $.ajax({
        type: "POST",
        data: "pageCode=" + pageCode,
        url: "/Admin/GoodsCatPager",
        async: true,
        success: function (data) {
            ...
            var goodscat = data.goodscat;
            var tr = "";
            for (var item in goodscat) {
                tr += "<tr>";
                tr += "<td>" + goodscat[item]["goods_catNam"] + "</td>" + "<td>" + goodscat[item]["catdesc"] + "</td>";
                var state = goodscat[item]["isenable"] == "1" ? "可用" : "已禁用";
                tr += "<td>" + state + "</td>";
                tr += "<td><input type='hidden' value=" + goodscat[item]["goods_catId"] + " /><a class='delgoodscat'  href='javascript:void(0);'>删除</a></td>";
                tr += "</tr>";
            }
...
$(function () {
    ...
    //绑定删除商品分类事件
    $(".table").on("click", ".delgoodscat", function () {
        ...
        swal({
            ...
            $.ajax({
                url: 'DelGoodsCat',
                data: 'goodsCatId=' + newsid,
                type: 'post',
                cache: false,
                success: function (data) {
                    data = eval("(" + data + ")");
                    var $toast = toastr['success'](data["msg"]);
                    getGoodsCat(); //重新查询首页，从第一页开始
                }
            });
        });
    });
