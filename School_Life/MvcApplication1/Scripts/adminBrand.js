    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var item in brand) {
                tr += "<tr>";
                tr += "<td>" + brand[item]["brandname"] + "</td>" + "<td>" + brand[item]["goods_catNam"] + "</td>";
                var state = brand[item]["isenable"] == "1" ? "可用" : "已禁用";
                tr += "<td>" + state + "</td>";
                tr += "<td><input type='hidden' value=" + brand[item]["grandid"] + " /><a class='delbrand'  href='javascript:void(0);'>删除</a></td>";
                tr += "</tr>";
            }
...
$(function () {
    ...
    //绑定删除商品分类事件
    $(".table").on("click", ".delbrand", function () {
